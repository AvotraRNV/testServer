...
// --- Route pour lister les fichiers dans uploads ---
app.get('/uploads', (req, res) => {
  fs.readdir(uploadFolder, (err, files) => {
    if (err) {
      return res.status(500).send('Impossible de lire le dossier uploads');
    }
...
app.delete('/uploads/:filename', (req, res) => {
  ...
  fs.unlink(filePath, (err) => {
    if (err) {
      ...
      return res.status(404).json({ success: false, message: 'Fichier non trouvé ou erreur lors de la suppression.' });
    }
    res.json({ success: true, message: `Fichier ${filename} supprimé.` });
  });
});
